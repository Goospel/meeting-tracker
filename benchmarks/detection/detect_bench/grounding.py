...
def _pick(indices: list, transcript: list, speaker, time_sec) -> int:
    ...
    def key(i):
        ...
        sd = _num(getattr(seg, "start_sec", 0.0))
        dt = abs(sd - ts) if (ts is not None and sd is not None) else 0.0
        return (spk, dt, i)
...
def _span_grounding(qc: str, transcript: list, speaker, time_sec) -> frozenset:
    ...
    if ts is not None:                      # 숫자 time 힌트가 있으면 최근접 창만
        def dist(c):
            sd = _num(getattr(transcript[c[0]], "start_sec", 0.0))
            return abs(sd - ts) if sd is not None else float("inf")
...
def ground_quote_span(quote: str, transcript: list, *, speaker=None, time_sec=None) -> frozenset:
    ...
    **포함하는** 경계 창이 있을 때만 span으로 확장(토큰이 한 세그먼트에 몰려도 파트너 세그먼트를
    ...
    if cands:
        ...
        if span and single in span:         # 그 세그먼트를 '포함'하는 창이면 확장
            return span
...
def resolve_flag_segments(flag, transcript: list) -> tuple[frozenset, list]:
    ...
    for st in flag.statements:
        span = ground_quote_span(st.quote, transcript,
