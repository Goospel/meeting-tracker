...
**3단계(파이프라인 통합) 착수** — **Python 우선 MVP**, 판단 패널 워크플로우로 5-PR 분해 확정. **PR-1(상태머신 순수 코어) 완료**(2차 리뷰 결함 10종까지 반영, TDD 52·워크플로우 5축 검증 SOUND), 다음은 PR-2(영속화). 아래 [3단계](#3-파이프라인-통합--python-우선-mvp-착수) 참조.
...
- ⬜ **PR-2 영속화** — JobRepository(InMemory fake + sqlite3), 재시작 복원·멱등/낙관적동시성 가드. 전사·결과 JSON은 로우 저장(별도 StoragePort 미도입 — 리플레이엔 실 오디오조차 없음, S3는 실 인제스트 시점). **rehydrate 시 Job 단위 불변식 전수 검증**(로드 경계): PR-1 `_check_state_invariant`는 피해 경로(빈 전사/결과 위 재실행·커밋)만 None-거부로 막았으니, 여기서 빈 문자열('') ref·상태별 전 ref 계보(ANALYZING의 transcript 보존 등)까지 로드 시점에 마감한다.
