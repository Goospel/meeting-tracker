...
- ⏸ 실제 Claude API 실호출 실측(크레덴셜 대기 — 어댑터는 완성, 포트 스왑만) · 골든 3건째(경계 span·tier2 퍼지 grounding을 실제 스트레스하는 하드케이스 — 인접 동일화자 세그먼트) · 통계 판정층(stage-1과 공유).
...
4. 🔜 **다음** — 실제 Claude API **실호출 실측**(어댑터 완성, 크레덴셜만 대기 → 포트 스왑) 또는 ⓑ 1단계 통계 판정층 설계 문서화(크레덴셜 없이 방법론 확정 가능) 또는 골든 3건째(경계 span·tier2 퍼지 grounding 스트레스 — 인접 동일화자 세그먼트)
