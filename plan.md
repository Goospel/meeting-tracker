...
- ⚠️ **실측 투입 전 필수 보강 3종**(리뷰2에서 확인, mock에선 안 밟힘): ① 인접 두 세그먼트에 걸친 인용이 통째로 '할루시' 오분류(세그먼트 연결/분할 grounding 필요) ② 동일 발화가 전사에 반복되면 항상 첫 출현에 오귀속(speaker/time 힌트 미사용 — 정당한 골든도 거부됨) ③ 변형 type 라벨 1건이 run 전체 중단(per-flag 강등 또는 정규화층 필요).
