...
- **TDD** Red→Green, **146테스트**(107 → +39, labels 회귀 1 포함). e2e: 리플레이 픽스처 전사→pred→report 만점, 파서 적대입력(빈/숫자/dict 배열 선행, 래퍼 에코 전후, 단일 flag, 부분강등, 추출실패) 전량 무성오답 0. **왜**: 실측(크레덴셜) 전에 **어댑터·프롬프트·파싱**을 크레덴셜 없이 확정 — 크레덴셜 오면 포트만 스왑하면 실측 시작.
