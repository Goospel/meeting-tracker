...
### feat · 파이프라인 PR-2 영속화 — JobRepository(InMemory+sqlite3)·재시작 복원·낙관적 동시성·로드경계 계보검증 ([PR #18](https://github.com/Goospel/meeting-tracker/pull/18))
- 3단계 PR-2. `pipeline_core/repository.py` 신설 — `JobRepository`(평범한 베이스, detect.DetectorPort 스타일) + `InMemoryJobRepository`(fake) + `SqliteJobRepository`(stdlib sqlite3) + `get_store('memory'|'sqlite')` 팩토리(detect.get_detector 미러). **재시작 복원**: 새 인스턴스가 같은 파일 재오픈해 동일 frozen Job 복원(연산별 커넥션 = Windows 파일락 잔류·tmp 정리실패 회피). **낙관적 동시성**: `version` 컬럼 + `save(job, expected_version)`(불일치=StaleVersionError). **의도적 미도입**(YAGNI, PR1 RETRYING과 동일 판단): transitions 감사테이블·StoragePort — 리더가 PR5 전엔 없어 write-only 死코드.
- **로드/저장 경계 전수검증 `state.validate_persisted`**(단일 출처, save·get 대칭 호출) — PR-1이 로드 경계로 명시 이관한 잔여 2건(빈 문자열 '', ANALYZING transcript 계보)을 마감. `_STATE_REQUIRES`+`_ORDER`에서 계보 요구 ref를 **파생**(이중 기재 회피) → ANALYZING/DONE도 앞 단계 transcript를 물려받아 강제. 손상 rehydrate가 상태머신·스토어 진입 전 CorruptJob. TDD **Red 먼저 확인 후 Green**(state 계보 10 + repository 29 = pipeline 91).
- **적대적 리뷰 5각(정확성·계약·sqlite·계보·altitude)→후보별 검증→스윕**(16에이전트, CONFIRMED 5 반영): ① **[med] 포트 대체가능성** — validate가 계보 ref만 봐 id/audio_ref/attempts NOT NULL 미검사 → 같은 Job이 InMemory 성공·Sqlite raw `IntegrityError`(약속한 CorruptJob 아님)로 발산 → 필수 스칼라를 게이트에 추가해 균일 거부. ② **[med] 빈값 술어 str-only** — `value == ""`가 공백만("  ")·비-str(BLOB b'') 관통 → `_is_blank_ref`(`not isinstance(str) or not strip()`)로 균일화. ③ **[low] `:memory:` 비호환** — 연산별 커넥션이라 각 연산이 새 빈 DB → 생성자 거부(InMemory 안내). ④ **[low] `_COLUMNS` 死코드** 제거. ⑤ **[med] INSERT/락 경합 docstring 과장** — 버전 가드는 UPDATE '버전 이동'만 잡음, 동시 최초저장 UNIQUE 경합·busy_timeout은 동시 라이터 있는 stage5 하드닝 → docstring 정직화(투기적 멀티워커 코드 미도입). **defer**: all_jobs 전체실패(PR4 복원정책 미결)·advance/save '' 비대칭(문서화된 의도 분업). Red 먼저 확인 후 Green, zero-dep·TDD **101**. 리포 전체 **608**(pipeline 101·detection 229·stt 167·stats 111).
...
- **2차 리뷰(`/code-review ultra` 로컬 max-effort 폴백) 결함 수정**(동일 브랜치): 10앵글 파인더→검증→스윕으로 발견 10건을 TDD로 수정하고 **적대적 검증 워크플로우**(5축 독립 스켑틱)로 재검증. **correctness 2종**(실측 확인) — **F1** 터미널 멱등 비대칭: `PERMANENTLY_FAILED←RETRY` 재전달이 DONE+COMMIT과 달리 raise(at-least-once 흡수 자기모순)→`_TERMINAL_MAKER` 표로 두 터미널 대칭 흡수. **F2** 재개 시 진입 불변식 미강제: 손상 rehydrate가 빈 전사/결과 위에서 재-STT 회피 재개·`COMMIT→DONE`·`START_ANALYSIS`를 통과(유료 재실행·정확히-한번 seam 관통)→`_STATE_REQUIRES`(상태 진입 불변식 단일 출처)를 `advance()` 모든 진입점+재개 결과에서 `_check_state_invariant`로 균일 강제(특수케이스 대신 일반화=altitude). **정리** `_ORDER`를 `_FORWARD`에서 도출(이중 유지 제거). **배선** conftest `sys.path` 순서 교정(benchmarks/detection append로 top-level shadowing 차단)·배선 테스트가 해석 경로까지 단언·docstring 과장(프로덕션 import는 PR4) 교정. **테스트 갭** ANALYSIS_DONE 멱등·FAIL/RETRY의 ts·frozen 구체예외 단언 보강. 잔여 2건(빈 문자열 '', ANALYZING transcript 계보)은 로드 경계 소유한 **PR2 rehydrate 검증으로 이관**(피해 경로 아님). zero-dep·TDD **52테스트**(F1/F2·우회 5종 Red 먼저 확인 후 Green), 워크플로우 5축 검증 SOUND(mutation으로 가드 load-bearing 확인).
