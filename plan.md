...
다음 착수는 **Track A(합성 골든셋 1건)** — 크레덴셜 없이 스크립트→골든→채점기 관통까지 가능하고, TTS 렌더만 크레덴셜 대기.
...
- 🔜 **테스트 데이터 확보 (2트랙 · 녹음 불필요)** — 벤치마크의 병목은 오디오가 아니라 **신뢰 레퍼런스 전사(골든)**다. 조사 결과, 직접 녹음 없이 확보하는 최선은 2트랙 병행:
  - ⬜ **Track A (즉시)** — 합성: Claude 회의 스크립트(화자 3~4인, 치명토큰 밀집 + 의도적 번복) → **비-네이버 TTS**(Azure Neural / Google Chirp3 HD) 렌더 → 잡음·리버브·전화코덱·겹말 증강. 스크립트=골든이라 정답 100% 통제, 기존 `fixtures/golden/` 포맷에 그대로 꽂힘. ⚠️ 벤치 대상에 클로바 포함 → 렌더는 **반드시 비-네이버**(같은 벤더 음향 prior 편향 회피).
...
2. 🔜 **Track A 합성 골든셋 1건** — Claude 회의 스크립트 + 골든 JSON을 TDD로, CTER 채점기 end-to-end 관통 (TTS 렌더는 크레덴셜 준비 시)
