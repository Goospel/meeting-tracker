...
**3단계(파이프라인 통합) 착수** — **Python 우선 MVP**, 판단 패널 워크플로우로 5-PR 분해 확정. PR-1(상태머신 순수 코어) 진행 중. 아래 [3단계](#3-파이프라인-통합--python-우선-mvp-착수) 참조.
...
- ⬜ **PR-5 실패/재시도/크래시복구 정확히-한번** — 실패 주입→FAILED·failed_from 재개(재-STT 회피)·재개 시 감지 **정확히 1회**(유료 이중호출 차단)·terminal + 적대적 리뷰 라운드.
