...
> ⚠️ **이 문서는 원 제품 구상·인계 노트입니다(2026-07-11).** 아래에 기술된 파이썬 프로토타입
> (faster-whisper · Chart.js · `build_dashboard.py`)과 그 기술 스택은 **폐기되었습니다.**
> 현재 확정 스택(Spring · AWS ECS · 네이버 클로바 · React · Claude)은 저장소 루트 `README.md`를 보세요.
> 아래에서 여전히 유효한 것은 **제품 아이디어 · 감지 4종(모순/번복/미해결/재논의) · 데이터 스키마**뿐입니다.
