...
Track A **단일 소스 빌더 + 첫 합성 골든 완료**(크레덴셜 없이 스크립트→골든→채점기 관통, max-effort 코드리뷰 반영, 129테스트). 남은 건 TTS 렌더(크레덴셜 대기).
